...
async def main():
    ...
    client = get_client()
    await client.WriteSchema(WriteSchemaRequest(schema=SCHEMA_ZED))
    ...
    import_requests = await client.BulkImportRelationships((req for req in requests))
    ...
    relationships = []
    async for resp in export_requests:
