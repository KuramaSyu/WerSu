...
delete, link / unlink) and enforces the right permission for every
operation.  Links are polymorphic over the target's kind -- see
:data:`~src.api.services.attachment_facade.LinkTargetType`.
"""
...
class AttachmentFacadeImpl(AttachmentFacadeABC):
    ...
    async def _check_link_permission(
        ...
        :meth:`link_attachment` ``"note"`` branch).
